...
describe("desktop update UI helpers", () => {
  ...
  it("includes the downloaded version in the install confirmation copy", () => {
    ...
    ).toContain("Install update 1.1.1 and restart Astrolabe?");
  });
  ...
  it("falls back to generic install confirmation copy when no version is available", () => {
    ...
    ).toContain("Install update and restart Astrolabe?");
  });

  it("adds the install warning on Windows", () => {
    expect(
      getDesktopUpdateInstallConfirmationMessage(
        {
          availableVersion: "1.1.0",
    ...
    ).toContain("On Windows, Astrolabe may remain closed for several minutes");
  });
});

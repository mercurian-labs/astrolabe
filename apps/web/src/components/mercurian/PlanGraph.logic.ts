...
export interface NavigatorLayout {
  ...
  readonly edges: ReadonlyArray<NavigatorEdge>;
  readonly laneCount: number;
}

const EMPTY_NAVIGATOR_LAYOUT: NavigatorLayout = { rows: [], edges: [], laneCount: 0 };

/**
 * The navigator: one row per commit in append order, each on a lane, with the
 * edges between them — the git-graph reading, and the one you move through.
 * Rows in time order are the easier walk, and rows are the thing you pick.
 *
 ...
 * reserved for it and closes the rest. A merge is drawn once, where its lanes
 * reunite — nothing here renders a commit twice.
 */
export function navigatorLayout(graph: PlanGraph): NavigatorLayout {
  if (graph.nodes.length === 0) return EMPTY_NAVIGATOR_LAYOUT;
  ...
  const edges = graph.nodes.flatMap((node) =>
    node.parents.flatMap((parentId): ReadonlyArray<NavigatorEdge> => {

...
/**
 * The Archived page's whole shape: archived threads grouped by their project.
 *
 ...
 * order so the two read the same way; threads within one are most-recently
 ...
 * somehow absent, so an unstamped thread still sorts rather than jumping about.
 */
...
/**
 * Which verbs an archived row offers. Restore always; delete only for a thread
 * that is still fully private — "Delete is not offered here for published
 * threads" is the same rule the tree row reads, from the same helper.
 */

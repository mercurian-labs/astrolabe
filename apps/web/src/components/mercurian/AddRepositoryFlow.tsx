import type { MercurianRepository, SourceControlDiscoveryResult } from "@t3tools/contracts";
...
export function AddRepositoryFlow({
  ...
  const handleAdded = useCallback(
    ...
    [onAdded, returnToPicker],
  );

  return (
    <>
      {renderHeader?.(mode === null ? "Add a repository" : modeTitle(mode))}
      {mode === null ? (
        <ModePicker onPick={pickMode} />
      ) : mode === "folder" ? (
        <FolderPath onBack={returnToPicker} onAdded={handleAdded} />
      ) : (
        <ClonePath mode={mode} onBack={returnToPicker} onAdded={handleAdded} />
      )}
    </>
  );
}
...
function FolderPath({
  ...
  }, [addRepository, isSubmitting, onAdded, path]);

import { MousePointer2Icon, PaintbrushIcon, PlusIcon } from "lucide-react";
import {
  useCallback,
  useEffect,
  useRef,
...
// A draft with no source theme starts as the standard Astrolabe look — the
...
export type ThemeEditorSurfaceRenderState = Readonly<{
  ariaLabel: string;
  content: ReactNode;
...
export function ThemeEditorSurface({
  open,
  onOpenChange,
  onSaved,
  editingTheme,
  ...
  restoreTheme: () => void;
  children: (surface: ThemeEditorSurfaceRenderState) => ReactNode;
}) {
  ...
  }, [activeAppearance, colorsByAppearance, isDraftSeeded, open, repaintNonce]);

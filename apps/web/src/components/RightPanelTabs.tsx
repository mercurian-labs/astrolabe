...
const SURFACE_DISABLED_REASONS = {
  browser: "Browser previews are only available in the Astrolabe desktop app.",
...
export function surfaceShortcutActionForKey<
  const Action extends { available: boolean; shortcut: string | null },
...
function SurfaceMenuItem(props: {
  ...
  const item = (
    <MenuItem
      ...
      {...(props.shortcut === null ? {} : { "aria-keyshortcuts": props.shortcut })}
    >
      {props.children}
      {props.shortcut === null ? null : <MenuShortcut>{props.shortcut}</MenuShortcut>}
    </MenuItem>
  );
  ...
  return (
    <div
      ...
      data-surface-launcher-keys={availableActions
        .flatMap((action) => action.shortcut ?? [])
        .join("")}
      className={cn(

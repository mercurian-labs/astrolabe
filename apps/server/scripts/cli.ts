...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/astrolabe (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@mercurian/astrolabe-<platform>.tgz` first, `astrolabe.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@mercurian");
      const launcherTarball = path.join(scopeDir, "astrolabe.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("astrolabe-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "astrolabe-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @mercurian/astrolabe-<platform> tarballs and then the astrolabe launcher to npm.",
  ),
);

 ...
 * ~/.astrolabe database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.astrolabe/userdata/state.sqlite`.
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedError<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .astrolabe directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedError<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.astrolabe database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .astrolabe directory. Defaults to `<worktree>/.astrolabe` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.astrolabe/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".astrolabe"));
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .astrolabe directory. Defaults to the current worktree's .astrolabe.",
      ),
    ),
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.astrolabe/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.astrolabe data, then run migrations.",
  ),
);

...
const canonicalizeProspectivePath = Effect.fn("AntigravityAdapter.canonicalizeProspectivePath")(
  function* (fileSystem: FileSystem.FileSystem, path: Path.Path, candidate: string) {
    ...
    while (true) {
      ...
      if (Option.isSome(canonical)) {
        return path.join(canonical.value, ...suffix);
      }
...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Resolve the nearest existing ancestor so new nested paths inherit the
    // canonical workspace root and cannot escape through a symlink.
    const real = yield* canonicalizeProspectivePath(input.fileSystem, path, resolved);

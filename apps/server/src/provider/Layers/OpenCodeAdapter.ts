...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const failPromptAdmissionRecovery = Effect.fn("failPromptAdmissionRecovery")(function* (
      ...
      const detail =
        "OpenCode accepted the prompt, but Astrolabe could not confirm its message or session status.";
    ...
    const emitPendingOpenCodeRequest = Effect.fn("emitPendingOpenCodeRequest")(function* (
      ...
      if (event.type === "permission.asked") {
        ...
        const normalized = normalizePermissionRequest(context, request);
        ...
        yield* emit({
          ...
          payload: {
            requestType:
              request === undefined
                ? "unknown"
                : normalizePermissionRequest(context, request).requestType,

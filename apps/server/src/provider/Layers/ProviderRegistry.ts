...
const shouldRetainMissingProviderModels = (provider: ServerProvider): boolean => {
  // An installed provider's successful ready snapshot is authoritative. In
  // particular, Claude's pending snapshot is not version-gated, while the
  // ready probe is, so retaining absent rows would resurrect unsupported
  // models from the pending inventory.
...
// An empty refresh is a weaker inventory signal for every driver except
// OpenCode, whose inventory legitimately becomes empty on logout or plugin
// removal. Preserve upstream's Antigravity pending/error handling through the

...
const version = "1.2.3";
const archiveName = `astrolabe-${version}-linux-x64.tar.gz`;
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      assert.equal(paths.entryPath, path.join(paths.versionDir, "astrolabe"));
  ...
  it.effect("refuses an archive whose checksum does not match the release", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.step, "verifying the astrolabe release archive checksum");

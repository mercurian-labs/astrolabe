...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    ...
    case "RelayAuthInvalidError":
      return "Run `astrolabe connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `astrolabe connect logout`, then run `astrolabe connect` again. Restart T3 Code after signing in.";

...
const linuxRuntime = "/home/theo/.astrolabe/runtime/versions/1.2.3/astrolabe";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.astrolabe",
  logPath: "/home/theo/.astrolabe/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/astrolabe.service",
};
...
it("reads the served T3 home back out of a rendered unit or plist", () => {
  const plan = (baseDir: string) => ({
    program: [`${baseDir}/runtime/versions/1.2.3/astrolabe`, "__service-launcher"],
    baseDir,
  ...
  expect(
    BootService.bootServiceBaseDirOf(
      BootService.renderBootServiceUnit(plan("/home/theo/.astrolabe")),
    ),
  ).toBe("/home/theo/.astrolabe");
...
const macRuntime = "/Users/theo/.astrolabe/runtime/versions/1.2.3/astrolabe";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.astrolabe",
  logPath: "/Users/theo/.astrolabe/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/com.mercurian.astrolabe.service.plist",
};
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.astrolabe/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const baseDir = path.join(home, ".astrolabe");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable astrolabe.service")
        control.enabled = true;
      if (!failed && command === "systemctl --user restart astrolabe.service")
        control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop astrolabe.service" ||
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              ...
              `astrolabe v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
  ...
  const makeService = (
    ...
    Effect.gen(function* () {
      ...
      return yield* BootService.make({
        ...
        cliVersion,
        host: { execPath: "/usr/bin/astrolabe" },
      });
    }).pipe(
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessExecutablePath, "/usr/bin/astrolabe"),
          Layer.succeed(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop astrolabe.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now astrolabe.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop astrolabe.service",
                "systemctl --user restart astrolabe.service",
              ]
            : [
                "launchctl bootout --wait gui/501/com.mercurian.astrolabe.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain("versions/1.2.4/astrolabe");
      expect(
        commands.filter(
      ...
      // the reason is named so `astrolabe service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user enable astrolabe.service",
        "systemctl --user restart astrolabe.service",
      ]);
    }),
  );
  ...
  it.effect("restart leaves a service that serves another T3 home alone", () =>
    Effect.gen(function* () {
      ...
      const other = yield* makeService(undefined, "1.2.3", path.join(otherHome, ".astrolabe"));
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.mercurian.astrolabe.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(
        timeouts.get("launchctl bootout --wait gui/501/com.mercurian.astrolabe.service"),
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.mercurian.astrolabe.service",
        "launchctl enable gui/501/com.mercurian.astrolabe.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.mercurian.astrolabe.service";

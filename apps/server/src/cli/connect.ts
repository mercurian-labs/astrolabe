...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `astrolabe connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `astrolabe connect link` to enable T3 Connect."
      : !status.linked
  ...
  return [
    ...
    "",
    "This is saved setup, not a live connection check. Check the background service with `astrolabe service status`.",
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `astrolabe connect unlink` again when the relay is reachable.",
      );
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `astrolabe connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `astrolabe connect link` before the

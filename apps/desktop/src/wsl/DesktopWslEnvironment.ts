...
export class DesktopWslEnvironment extends Context.Service<
  DesktopWslEnvironment,
  {
    ...
    // Proves a staged self-contained runtime can run (`<root>/astrolabe --version`)
...
// Holds the sha256 of the runtime's `astrolabe` executable, written when the install
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    // The runtime is a self-contained `astrolabe` executable with Node inside, so the
    // readiness proof is the same one the SSH runner and the CLI installers
    // use: the file is executable and `astrolabe --version` exits 0. That covers the
    // truncated-binary and wrong-arch cases without a separate native probe.
    "runtime_entry_runs() {",
    '  [ -x "$1/astrolabe" ] && "$1/astrolabe" --version >/dev/null 2>&1',
    "}",
    // Hashing the entry is what tells a working cache from one whose `astrolabe` was
    ...
    `  sha256sum "$1/astrolabe" 2>/dev/null | cut -d ' ' -f 1`,
    "}",
    "runtime_is_ready() {",
    '  [ -f "$ready_marker" ] &&',
    ...
    // The release archive has one top-level `astrolabe-<version>-linux-<arch>/`
    // directory; strip it so the executable lands at `$runtime_root/astrolabe`.
    ...
    "  printf 'WSL runtime archive does not contain a working astrolabe executable\\n' >&2",
    "  exit 1",
    "fi",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.astrolabe/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
export const buildWslRuntimeProbeScript = (linuxAppRoot: string) =>
  [
    ...
    `${shellQuote(`${linuxAppRoot}/astrolabe`)} --version >/dev/null 2>&1`,
  ].join("\n");
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode !== 0) {
      ...
      return {
        ok: false,
        reason: `${linuxAppRoot}/astrolabe --version failed (exit ${probe.exitCode})${trimmedTail ? `: ${trimmedTail}` : ""}`,
      } as const;
    }

...
// Stands in for the release's self-contained `astrolabe` executable: the install
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    ...
    expect(script).toContain('runtime_parent="$HOME/.astrolabe/wsl-runtime"');
    ...
    // directory; stripping it puts the executable at `$runtime_root/astrolabe`.
  ...
  it("treats a runtime whose executable no longer runs as a cache miss", () => {
    ...
    expect(script).toContain('  [ -x "$1/astrolabe" ] && "$1/astrolabe" --version >/dev/null 2>&1');
  ...
  // A swapped or half-written `astrolabe` can still exist and even still answer
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(`  sha256sum "$1/astrolabe" 2>/dev/null | cut -d ' ' -f 1`);
    expect(script).toContain(
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.astrolabe/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.astrolabe/wsl-runtime/1.2.3-x64",
    );
  ...
  it("never deletes a runtime another backend is running from", () => {
    ...
    // The running backend's argv holds `<runtime>/astrolabe`, so
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain(
      'rm -f "$HOME/.astrolabe/wsl-runtime/1.2.3_x64/.t3code-wsl-runtime-ready"',
    );
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        'stage="$work/stage/astrolabe-0.0.0-linux-x64"',
        'mkdir -p "$stage/node_modules/node-pty/build/Release" "$work/home"',
        `printf '%s' ${sh(SERVER_ENTRY_SOURCE)} > "$stage/astrolabe"`,
        'chmod +x "$stage/astrolabe"',
        `printf '%s' 'pty-native-payload' > "$stage/node_modules/node-pty/build/Release/pty.node"`,
        `tar -czf "$work/wsl-runtime.tar.gz" -C "$work/stage" astrolabe-0.0.0-linux-x64`,
        `printf 'work:%s\\n' "$work"`,
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.astrolabe/wsl-runtime`,
      runtimeRoot: `${work}/home/.astrolabe/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.astrolabe/wsl-runtime/${runtimeId}/astrolabe`,
      installScript,
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        'sh -c "sleep 30" "$runtime_root/astrolabe" >/dev/null 2>&1 &',
        "active_pid=$!",
        "sleep 0.1",
        fixture.installScript(),
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.astrolabe/wsl-runtime"',
        ...
        'sh -c "sleep 30" "$runtime_parent/sha256-active/astrolabe" >/dev/null 2>&1 &',
        "active_pid=$!",
        "(",

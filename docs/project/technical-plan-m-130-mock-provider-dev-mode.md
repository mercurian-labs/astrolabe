...
_Generated from the Goal/AC of Linear issue M-130 (see the issue for the full AC). Built on the t3code-fork base as it stands at M-128, under [ADR 004](../architecture/fork-baseline.md) (Mercurian code additive; minimal edits in upstream-owned files — this plan spends its budget on four small additive edits, named below). Regenerated after M-128 landed: the workspace planning **default** retired in favor of last-used seeding, which changes only how the mock becomes what a fresh dev workspace plans under (Design §4); every other section survived M-128 untouched. The mock rides the fork's generic provider layer, so it is not Mercurian-only machinery: in dev it is equally usable from t3code thread surfaces — a side effect, not a goal._

**Goal, in one sentence:** ship a `mock` provider driver that is registered only when a dev flag says so, always reports installed/ready/signed-in with a small model list, synthesizes deterministic runtime events (streaming text, stop, plan revision, structured question, grounding) instead of spawning any CLI — and, on a dev server whose workspace has never planned under anything, seed the workspace's last-used planning model to it so a fresh dev workspace plans out of the box.
...
- **M-128 retired the planning default; the seed's landing spot is now last-used.** `WorkspaceSettingsStore` ([mercurian/workspace/WorkspaceSettingsStore.ts](../../apps/server/src/mercurian/workspace/WorkspaceSettingsStore.ts)) no longer has a public setter — its one writer is `recordLastUsedPlanningModel(selection)`, and the stored `planningModel` now means "the pair most recently stamped on a turn-opening human message." Null still means "no one has ever planned here," which is exactly the only state a seed may overwrite. The composer computes `flip ?? standing ?? lastUsed` ([PlanningSpace.tsx:246](../../apps/web/src/components/mercurian/PlanningSpace.tsx)) for live plans and `draft ?? lastUsed` for unborn ones, and the almagest Assistant note's second amendment records the design intent: "in a fresh workspace with no planning history, the composer asks for a choice there before the assistant can reply, and everything after seeds from use." The dev seed slots underneath that exact mechanism — it makes a fresh dev workspace look like one that last planned under the mock, so the composer never has to ask.
...
| Workspace planning-model state is last-used, written only by the product (`recordLastUsedPlanningModel`); no configured default exists anywhere                           | `WorkspaceSettingsStore.ts`, M-128 plan doc                | High       |
...
Client visibility: the Settings → Providers list renders from `PROVIDER_CLIENT_DEFINITIONS` ([providerDriverMeta.ts](../../apps/web/src/components/settings/providerDriverMeta.ts)), a hardcoded client-side allowlist — a driver the server reports but the client has never heard of is invisible. Mock joins that list and, like cursor before it, renders only when a live instance for the slot exists (the `SNAPSHOT_DISCOVERED_DEFAULT_DRIVERS` path in [ProviderSettingsPanel.tsx](../../apps/web/src/components/settings/ProviderSettingsPanel.tsx)), so a build without the mock registered still shows nothing. It is kept out of the add-instance options, since only the dev server bootstraps it.
...
### 4. Seeding what a fresh dev workspace plans under

M-128 removed the configured planning default: the composer's effective pair is `flip ?? standing ?? lastUsed`, and in a fresh workspace all three are empty, so the composer asks for a choice before the assistant can reply. That ask is precisely the configuration step the AC forbids in dev — so the dev seed targets the last slot in that chain.

A small boot effect in the Mercurian block of [server.ts](../../apps/server/src/server.ts) ([mercurian/workspace/MockPlanningModelSeed.ts](../../apps/server/src/mercurian/workspace/MockPlanningModelSeed.ts)): when `ServerConfig.mockProviderEnabled` and `WorkspaceSettingsStore.getSnapshot` reports `planningModel: null`, call `recordLastUsedPlanningModel({ provider: "mock", model: "mock-default" })` — the fresh dev workspace behaves as if it last planned under the mock. Null is the only state overwritten, and null now means "no turn-opening message has ever been stamped here," so the seed can never displace a real record. "A default, not an override" holds by M-128's own mechanism: the seed only fills the bare-history slot; the first turn that runs under any pair — mock or real — stamps ancestry and re-records last-used, and branches with a record derive from ancestry, never from the seed. Explicitly choosing a real provider in the composer picker therefore permanently retires the mock seed for everything that descends from that message.
...
With the flag off later (a previously-seeded workspace reopened without the mock), the pair degrades exactly like any recorded pair whose provider this machine lacks: `resolvePlanningModel` reports no-instance, the composer gates, and M-128's picker preserves the slug as a disabled option rather than erroring — the last AC's degradation path, unchanged.

### 5. What this plan deliberately does not touch

Planning assistant, planning store, contracts, and every client surface beyond the Settings provider-card allowlist (§2): the mock arrives through the same snapshots and events as real providers, so pickers, gates, resolution, and timelines need nothing. There is no Settings planning-model row anymore (M-128 deleted it), so the seed has no Settings surface at all — the composer picker is where the seeded pair shows. `TestProviderAdapter` and the integration harness: untouched. Mobile: parked.

## Implementation Checklist

Status reflects the branch as of the M-128 merge: everything landed in `863b6d030` and `faa871b63` except the seed rework, which M-128 broke.

- [x] `apps/server/src/cli/config.ts` + `config.ts` — `T3CODE_MOCK_PROVIDER` → `ServerConfig.mockProviderEnabled` (additive upstream edit).
- [x] `scripts/dev-runner.ts` — default the var to `"1"` when unset, beside `T3CODE_BUNDLED_DEV` (additive upstream edit).
- [x] `apps/server/src/provider/Layers/MockAdapter.ts` (+ test) — self-driving `ProviderAdapterShape`: scripted streaming, `/question`, `/ground`, `/revise` behaviors, final-segment steering, immediate abort; no spawn, no network.
- [x] `apps/server/src/provider/Drivers/MockDriver.ts` (+ test) — constant-ready snapshot with the two mock models; `metadata.bootstrapWithoutSettings: true`.
- [x] `apps/server/src/provider/ProviderDriver.ts` — optional `bootstrapWithoutSettings` metadata flag (additive upstream edit).
- [x] `apps/server/src/provider/Layers/ProviderInstanceRegistryHydration.ts` (+ test) — effective-drivers parameter; defaultConfig bootstrap branch for flagged drivers (additive upstream edit).
- [x] Wire `effectiveDrivers` where the registry layer meets `ServerConfig`.
- [x] Web: mock joins `PROVIDER_CLIENT_DEFINITIONS`; snapshot-discovered default-driver rendering in `ProviderSettingsPanel.tsx`; excluded from add-instance options.
- [ ] `apps/server/src/mercurian/workspace/MockPlanningModelSeed.ts` (+ test) — rework for M-128: seed via `recordLastUsedPlanningModel` when the stored pair is null; update the module doc to last-used semantics.
- [x] Docs: `docs/internals/overview.md` dev-workflow note (mock provider, the env var, opting out); **no `docs/user/` entry** — the mock never exists in a shipped build.
...
- [ ] Commits: `feat(scope): … (M-130)` on branch `venk/m-130-mock-provider-for-dev-mode-mock-planning-replies-with-zero`.

## Test Plan

Unit — server (`vp test run` on the touched files):

- [x] `MockAdapter.test.ts` — a turn streams deltas then completes; `/question` pauses with the fixed question and resumes on answer; `/ground` emits item events before text; steering reads only the final reply segment of a rebuilt-session preamble; interrupt yields `turn.aborted`; two sessions share no state; scripts are byte-deterministic across runs.
- [x] `MockDriver.test.ts` — snapshot constant-ready (installed, authenticated, two models, no advisory); `create` twice → independent instances.
- [x] `ProviderInstanceRegistryHydration.test.ts` — flagged driver with no legacy mirror bootstraps its default instance; unflagged drivers don't; an explicit `providerInstances["mock"]` entry wins; with the mock absent from the effective list, a saved `mock` entry surfaces as the unavailable shadow (existing behavior, now pinned for this driver).
- [ ] `MockPlanningModelSeed.test.ts` — null last-used + flag → recorded once (visible via `getSnapshot`); non-null → untouched; flag off → untouched.

Manual (dev app, `vp run dev` with a fresh worktree `.t3`):

- [ ] Fresh dev workspace: the composer picker comes up already showing Mock (mock-default); create a plan, send — a mock reply streams with zero configuration; Settings → Providers shows a Mock card, ready.
- [ ] Stop mid-reply → interrupted mark; `/question` → question card, answering resumes; `/ground` → grounding fold renders.
- [ ] Pick a real provider/model in the composer picker and send — the turn stamps that pair, and the mock does not reassert itself on descendants or on the next fresh plan (last-used now carries the real pair).
- [ ] `T3CODE_MOCK_PROVIDER=0 vp run dev` on a previously-seeded workspace — no mock anywhere; the recorded mock pair surfaces as unavailable in the picker and the composer gates (nothing errors).
...
- **M-128 landed and reshaped the seed** (Design §4): the workspace default the original plan seeded no longer exists; the seed now pre-records last-used, which is a strictly better fit for "a default, not an override" — the product's own bare-history rule enforces it instead of the seed's idempotence check. M-131's gate never fires on the mock (always signed-in). Neither issue is a dependency.

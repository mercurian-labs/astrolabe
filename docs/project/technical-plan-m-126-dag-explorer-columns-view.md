...
1. **Fork separator bar** — the fork block's bare top border becomes a single separator line with the label sitting in a gap in the line: "forks" beside the `lucide-react` `ArrowDown` glyph, in the app's muted label treatment. One line, not a filled bar. It reads as "the run continues into one of these".
...
3. **Right-anchored panes, flexible last pane** — the columns container packs content to the sidebar's right edge (auto left margin on the first pane: packs right while content fits, degrades to ordinary left-origin horizontal scrolling when it overflows). Every open pane flexes between the fixed reading width (min) and a maximum (~1.5×), so spare width distributes across the panes being read and no width leaves a dead gap; when a single pane is the only one open, its maximum stretches further (~416px) so the band of widths just before the next pane can reopen stays filled too. Strips stay fixed-width. Compression is width-aware: panes auto-expand right to left while their reading width fits the measured container (one ResizeObserver, event-driven), the rest compress to strips; the manual strip click still force-expands one more beyond the auto-fit set.
4. **Per-view sidebar cap** — the stored width key stays singular; each view renders `min(storedWidth, viewCap)`. The thread view gets a fixed cap; the columns view's cap is the fully-expanded content width (every pane at reading width plus the shared flexible room; pane borders sit inside the box widths, so no separate separator term) so the sidebar can grow until every strip has reopened, and no further; the graph view and plan artifact keep the existing maximum. `PlanningSpace` learns the explorer's active view reactively through the existing same-window `useLocalStorage` subscription on `mercurian:dag-explorer-view:v1` — no state lifting, same decode-failure fallback. The view cap is passed to the resize hook as its live maximum, so the displayed width, the drag baseline, and the persisted value all agree: dragging responds immediately in a capped view (no dead travel), releasing persists the dragged value, and a wider width from an uncapped view survives view switches until the user drags inside a capped one.
5. **Main-pane minimum, overlay fallback** — a constant names the conversation's minimum readable width. At `sm+`, when the planning space is too narrow to fit that beside the right pane's effective width, the right pane renders as a right-anchored overlay above the conversation (existing border/background treatment, resize handle still live) instead of squeezing it. The pane's width — dragged or capped — is additionally bounded by the planning space's own width, so the overlay never extends past the conversation's left border underneath the app sidebar. Measurement is event-driven only (one observer on the space's root; no loops). Below `sm`, the stacked layout is unchanged.

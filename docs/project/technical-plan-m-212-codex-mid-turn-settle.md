...
_Addendum after the AC walk and review (2026-09-04): two things the plan above did not foresee landed on the same branch as separate commits. The walk found that the live client reducer dropped the per-repository `repositories` groups on `thread.turn-diff-completed` (a pre-existing gap from M-196), fixed in `packages/client-runtime/src/state/threadReducer.ts`. A GPT-6 Astra review found that the SQL projection (`ProjectionPipeline.ts`) let a placeholder that lands after the real capture overwrite the captured ref, status, and files; the in-memory projector already guarded against this, and the placeholder path used to repair it by re-capturing, which the session gate removed. The pipeline now refuses a `missing` placeholder over a captured checkpoint and a non-partial event over a partial capture, and the client reducer mirrors the partial guard._

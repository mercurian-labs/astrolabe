...
> For maintainers. Using Astrolabe? See [docs/user](../user/).

A provider is the agent runtime that does the actual work. Astrolabe supports several, and the
...
Astrolabe does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before Astrolabe's next refresh sees them.

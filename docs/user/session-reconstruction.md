...
Replies that continue the same session refer to its original reconstruction. This describes the context supplied when that session started; it does not describe later compaction performed privately by the provider.

Older replies and sessions whose reconstruction was not recorded show no reconstruction section. If recorded detail cannot be loaded, the checkpoint offers a retry instead of guessing.

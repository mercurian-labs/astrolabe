...
| Task                            | Command                                             |
| ------------------------------- | --------------------------------------------------- |
| Install and start               | `npx @mercurian/astrolabe@latest service install`   |
| Inspect status and log location | `npx @mercurian/astrolabe@latest service status`    |
| Update or repair                | `npx @mercurian/astrolabe@latest service update`    |
| Stop and remove from startup    | `npx @mercurian/astrolabe@latest service uninstall` |

Uninstalling the service leaves your projects, threads, and settings intact.

Install and update use the version of the CLI you invoke. For nightly, use
`npx @mercurian/astrolabe@nightly service update`; replace `nightly` with an exact version to pin
...
It places `astrolabe` in `~/.local/bin` and reuses the same download when you later
run `astrolabe service install`. It follows the stable train by default; set
...
and are never offered as updates; the installer and `astrolabe update` only take you
there when you ask for the channel explicitly, and warn you when they do.

Once a self-contained `astrolabe` is installed, `astrolabe update` moves the machine to a
newer one without npm: it downloads the newest release on the channel the
running `astrolabe` came from, verifies it, and points the `astrolabe` launcher at it. When
...
`astrolabe service update`. From a script there is no prompt, so pass `--yes` to
...
yourself. Pass an exact version (`astrolabe update 0.0.41-preview.20260912.1595`) to
...
`astrolabe uninstall` reverses the install script: it shows what it found (the
background service, the `astrolabe` launcher, every downloaded version under
`~/.astrolabe/runtime`), asks once, and removes them. Your projects, threads, and
settings under `~/.astrolabe/userdata` are kept; delete that directory yourself if
...
Start with `astrolabe service status` on the host. It prints the log path and, on Linux,
...
identity. Without administrator access, run `astrolabe serve` in a terminal and keep
that session open.
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status astrolabe.service`, then use the repair command printed by T3 Code.                  |
| `restart-pending`                       | A newer version is installed but the service still runs the previous one. Run `astrolabe service restart`.                     |

On macOS, check **System Settings → General → Login Items** if the service no
longer starts at login. If agent work cannot access Desktop, Documents, or
Downloads, it may need Full Disk Access for the Node executable listed in
`ProgramArguments` in
`~/Library/LaunchAgents/com.mercurian.astrolabe.service.plist`.

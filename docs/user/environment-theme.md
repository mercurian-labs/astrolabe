...
machine running your Astrolabe server does that, its themes appear in the theme library alongside
your own, and Astrolabe follows them: change the desktop theme and Astrolabe retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. Astrolabe Mobile keeps its own
...
the standard Astrolabe look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Astrolabe state
...
exported from Astrolabe (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Astrolabe generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Astrolabe theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Astrolabe does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Astrolabe never reads

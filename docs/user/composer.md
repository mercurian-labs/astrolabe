...
Messages can contain up to 120,000 characters. If a draft is longer, Astrolabe keeps it in the
...
Astrolabe from other apps through the system share sheet. Mobile keeps a local copy of each draft
...
Astrolabe remembers the last provider, model, and model options you selected and reuses that
...
Model options shown as provider defaults remain display values until you choose them in Astrolabe.
Astrolabe only sends options you selected explicitly, so an unset reasoning level or service tier can
...
readable and Astrolabe shows a warning.
...
On web, desktop, and mobile, select a link to an image or video to open it inside Astrolabe.
Workspace image and video links open the file viewer. Links to media outside the workspace
open a media preview.
Videos opened from the file explorer or a file-viewer tab also play inside Astrolabe. They
...
Astrolabe serves the original file without adding it to attachment storage. If that file is moved
...
runs in an isolated frame with no access to your Astrolabe session. On desktop, the integrated
...
project exists there. If it does not, Astrolabe selects another environment that has the project.
...
and attachments. Astrolabe deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, Astrolabe hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. Astrolabe sends it to each provider in
...
them only there. Astrolabe's own commands, such as `/model`, and skills stay available on
any line.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. Astrolabe opens another new thread and shows an **Open** action for the

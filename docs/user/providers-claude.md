...
This guide is for people who want to use more than one Claude setup in Astrolabe. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Astrolabe](./install.md).
...
In Astrolabe Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Astrolabe uses Claude Code's normal config directory.

When you set this field, Astrolabe points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Astrolabe
...
Astrolabe looks for Claude skills in the Claude config directory's `skills` folder and
`<workspace>/.claude/skills`, the two places Claude Code loads them from.

If the same skill name exists in both folders, the one in the Claude config directory wins, the
same way Claude Code resolves it.
...
`~/.claude_personal_home/.claude`, which is not where Astrolabe looks.

Then add another Claude provider in Astrolabe:
...
Astrolabe only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Astrolabe keeps separate config directories isolated
...
Add or edit a Claude provider in Astrolabe Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Astrolabe stores the value as a server secret and does not
...
Astrolabe does not need a special Claude Code Router provider. Treat the router as a Claude

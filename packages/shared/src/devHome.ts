 ...
 * `~/.astrolabe` that a user's installed T3 Code runs against.
 *
 * A linked git worktree gets its own (gitignored) `.astrolabe`: feature work in a
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".astrolabe");
  });

/**
 * `dist/bin.mjs` of the `astrolabe` npm package: the entry point boot-service
 ...
 * executable in the sibling `@mercurian/astrolabe-<platform>-<arch>` package.
 *
...
const executableName = process.platform === "win32" ? "astrolabe.exe" : "astrolabe";
const executable = join(dirname(require.resolve("@mercurian/astrolabe-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("astrolabe: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {

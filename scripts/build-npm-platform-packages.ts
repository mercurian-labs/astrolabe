 ...
 * behind `npx @mercurian/astrolabe` / `npm i -g @mercurian/astrolabe`: one `@mercurian/astrolabe-<platformKey>` package per
 * archive holding the archive's contents verbatim, plus the `astrolabe` launcher
 ...
 *
 *   @mercurian/astrolabe-<platformKey>/      archive contents flattened + package.json
 *   @mercurian/astrolabe-<platformKey>.tgz   the same tree as an npm tarball
 *   @mercurian/astrolabe/       launcher: package.json, bin/astrolabe.js, README.md
 *   astrolabe.tgz                          the launcher as an npm tarball
 *
...
export const NPM_PLATFORM_PACKAGE_SCOPE = "@mercurian";
export const NPM_LAUNCHER_PACKAGE_NAME = "@mercurian/astrolabe";
...
export function npmPlatformPackageName(platformKey: CliArchivePlatformKey): string {
  return `${NPM_PLATFORM_PACKAGE_SCOPE}/astrolabe-${platformKey}`;
}
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `Astrolabe CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: ["astrolabe", "astrolabe.exe", "client", "resource-monitor", "node_modules"],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The Astrolabe CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/mercurian-labs/astrolabe",
    "",
  ].join("\n");
}

/** package.json for the `astrolabe` launcher. No engines: bin/astrolabe.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "Astrolabe CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { astrolabe: "./bin/astrolabe.js" },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx @mercurian/astrolabe` runs. Plain CommonJS with no dependencies so it
...
let packageDir;
try {
  packageDir = dirname(require.resolve("${NPM_PLATFORM_PACKAGE_SCOPE}/astrolabe-" + key + "/package.json"));
} catch {
  process.stderr.write(
    [
      "astrolabe: no Astrolabe CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall @mercurian/astrolabe so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/mercurian-labs/astrolabe/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "astrolabe.exe" : "astrolabe");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("astrolabe: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32") ? "astrolabe.exe" : "astrolabe";
...
/** Writes the launcher package (package.json, bin/astrolabe.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, "bin/astrolabe.js");
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.string("archives-dir").pipe(
      Flag.withDescription(
        "Directory holding the release's astrolabe-<version>-<platform> archives.",
      ),
    ),
  ...
  Command.withDescription(
    "Build the astrolabe launcher and @mercurian/astrolabe-<platform> npm packages from CLI release archives.",
  ),
);

...
#   T3CODE_INSTALL_BIN_DIR   where astrolabe.exe is linked (default: ~\.local\bin)
...
$t3Home = if ($env:T3CODE_HOME) { $env:T3CODE_HOME } else { Join-Path $HOME ".astrolabe" }
$binDir = if ($env:T3CODE_INSTALL_BIN_DIR) { $env:T3CODE_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "astrolabe install: $message"
  exit 1
}
...
if ($version -match '-preview\.') {
  Write-Warning "astrolabe $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set T3CODE_CHANNEL=stable (the default) for a supported build."
  if ($channel -ne "preview" -and -not $env:T3CODE_VERSION) {
    Fail "refusing a preview build that was not explicitly requested"
  }
}

$stem = "astrolabe-$version-win32-$arch"
$archive = "$stem.zip"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  Write-Host "astrolabe $version is already installed at $targetDir"
} else {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "astrolabe $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'npm install -g @mercurian/astrolabe@$version'"
      }
      throw
    }
    ...
    & (Join-Path $staging "astrolabe.exe") --version | Out-Null
...
$shim = Join-Path $binDir "astrolabe.cmd"
# UTF-8 without a BOM: cmd.exe reads the shim as-is, and ASCII would corrupt
# non-ASCII characters in the user's home path.
[System.IO.File]::WriteAllText($shim, "@echo off`r`n`"$(Join-Path $targetDir 'astrolabe.exe')`" %*", (New-Object System.Text.UTF8Encoding $false))
Write-Host "Installed astrolabe $version"
Write-Host "  $shim -> $(Join-Path $targetDir 'astrolabe.exe')"
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "Add $binDir to your PATH to run astrolabe."
}

...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub `astrolabe` that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  ...
  for (const key of KEYS) {
    const stem = `astrolabe-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "astrolabe"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "astrolabe"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "@mercurian/astrolabe-darwin-arm64",
          "@mercurian/astrolabe-linux-x64",
          "@mercurian/astrolabe",
        ],
      );
      ...
      const linuxDir = path.join(fixture.outputDir, "@mercurian/astrolabe-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@mercurian/astrolabe-linux-x64");
      ...
      assert.equal(Number((yield* fs.stat(path.join(linuxDir, "astrolabe"))).mode) & 0o111, 0o111);

      const darwinManifest = yield* decodeManifest(
        yield* fs.readFileString(
          path.join(fixture.outputDir, "@mercurian/astrolabe-darwin-arm64/package.json"),
        ),
      );
      assert.deepStrictEqual(darwinManifest.os, ["darwin"]);
      assert.deepStrictEqual(darwinManifest.cpu, ["arm64"]);

      const launcherDir = path.join(fixture.outputDir, "@mercurian/astrolabe");
      const launcherManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(launcherDir, "package.json")),
      );
      assert.equal(launcherManifest.name, "@mercurian/astrolabe");
      assert.equal(launcherManifest.version, VERSION);
      assert.deepStrictEqual(launcherManifest.bin, { astrolabe: "./bin/astrolabe.js" });
      assert.deepStrictEqual(launcherManifest.files, ["bin", "dist"]);
      assert.deepStrictEqual(launcherManifest.optionalDependencies, {
        "@mercurian/astrolabe-darwin-arm64": VERSION,
        "@mercurian/astrolabe-linux-x64": VERSION,
      });
      assert.isUndefined(launcherManifest.engines);
      assert.isTrue(yield* fs.exists(path.join(launcherDir, "bin/astrolabe.js")));
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@mercurian"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@mercurian/astrolabe-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/astrolabe$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(
          process.execPath,
          ["bin/astrolabe.js", "serve", "--port", "1234"],
          {
            cwd: launcherDir,
            env,
          },
        ...
        const unpack = yield* run(
          ...
          [
            "-xf",
            path.join(fixture.outputDir, "@mercurian/astrolabe.tgz"),
            "-C",
            installedLauncher,
          ],
          {
      ...
      const unsupported = yield* run(process.execPath, ["bin/astrolabe.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/mercurian-labs/astrolabe/releases");
    }),
  );
});

...
Astrolabe is in active development. There is no packaged release yet; run it from source as described below.

## Lineage

Astrolabe is a fork of [T3 Code](https://github.com/pingdotgg/t3code) by T3 Tools. It is MIT-licensed, preserves the full upstream history, and actively tracks upstream. See [LICENSE](./LICENSE) for both copyright notices.

The inherited Arch Linux AUR packaging lives under [`packaging/aur`](./packaging/aur).
...
Astrolabe uses Vite+, so install the global `vp` command-line tool first.
